...
#--- System related ----------------------------------------------
SYSTEMS = {
    'default': redis.Redis(host=settings.REDIS_HOST, port=settings.REDIS_PORT, password = settings.REDIS_PASSWORD or None),
    'test': redis.Redis(host=settings.REDIS_HOST, port=settings.REDIS_PORT, password = settings.REDIS_PASSWORD or None, db = 1)
}
...
def choose_default_system(name):
    SYSTEMS['default'] = SYSTEMS[name]
...
class SetFu:
    ...
    def sample(self, size):
        return [ get_redis(self.system).srandmember(self.name) for i in range (min (size, len(self))) ]
